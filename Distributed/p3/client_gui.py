...
class GuiPart:
    def __init__(self, master, queue, endCommand):
        ...
        self.info = tk.Label(master, text=self.info_data, width=20)
        self.request = tk.Button(master, text='Solicitar', command=lambda: self.turnOnIsRequest(), width=20)
        self.end = tk.Button(master, text='Done', command=endCommand, width=20)
    ...
    def processIncoming(self):
        ...
        while self.queue.qsize():
            try:
                msg = self.queue.get(0).decode().split(" ")
                if len(msg) == 3:
                    print(msg)
                    self.info_data = msg[2]
