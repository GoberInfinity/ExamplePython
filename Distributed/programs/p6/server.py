...
class Aplication:
    def __init__(self, master):
        ...
        self.thread8 = threading.Thread(target=self.getNewHourThread).start()
    ...
    def workerThread(self, n_thread):
        ...
        while True:
            ...
            if n_thread == 1 and g_hour and g_needs_hour_update:
                current_time = g_hour
                g_needs_hour_update = False
                print(f"UPDATING FROM MASTER {current_time}")
            if n_thread == 1:
                g_hour = current_time
