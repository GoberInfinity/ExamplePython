...
class SegmentTree:
    def __init__(self, array):
        ...
        self.build(index_current_vertex= 0, seg_tree_left_limit=0 , seg_tree_right_limit=self.len_arr-1)
    ...
    def build(self, index_current_vertex, seg_tree_left_limit, seg_tree_right_limit):
        if seg_tree_left_limit == seg_tree_right_limit:
            ...
            self.seg_tree[index_current_vertex] = self.seg_tree[left_child_vertex] + self.seg_tree[right_child_vertex]
    
    def querySum(self, query_start, query_end):
        return self._querySum(0, 0, self.len_arr-1, query_start, query_end)
        
    def _querySum(self, index_current_node, seg_tree_left, seg_tree_right, query_left, query_right):
        ...
        return self._querySum(left, seg_tree_left, middle, query_left, min(query_right, middle)) + \
            self._querySum(right, middle+1, seg_tree_right, max(query_left, middle+1), query_right)
    
    def updateValueOfIndex(self, index, new_val):
        self._updateValueOfIndex(0, 0, self.len_arr-1, index, new_val)
    
    def _updateValueOfIndex(self, index_current_node, seg_tree_left, seg_tree_right, index_val, new_val):
        if seg_tree_left == seg_tree_right: #[2..2]
            ...
            right = 2 * index_current_node +2
            if index_val <= middle:
                self._updateValueOfIndex(left, seg_tree_left, middle, index_val, new_val)
            else:      
                self._updateValueOfIndex(right, middle+1, seg_tree_right, index_val, new_val)
            self.seg_tree[index_current_node] = self.seg_tree[left] + self.seg_tree[right]
